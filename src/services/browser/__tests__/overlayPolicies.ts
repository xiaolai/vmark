 ...
 * here as *code*, and `overlayPolicies.test.ts` reads `App.tsx` and fails if any
 * overlay rendered there is missing a policy. Adding an overlay without deciding what
 * it does about the browser is now a build failure, not an oversight. Only that test
 * reads the table — the hook freezes unconditionally — so it lives under `__tests__/`
 * as the test infrastructure it is (feature-ledger plan, Phase 3).
 *
 * Policies:
 ...
 * @module services/browser/__tests__/overlayPolicies
 */
