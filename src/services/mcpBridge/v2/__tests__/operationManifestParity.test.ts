...
describe("bridge operation manifest parity", () => {
  ...
  it("sidecar wire types match the manifest exactly", () => {
    // The union is GENERATED from `operationSchemas.ts` since WI-15;
    // `core-types.ts` re-exports it. Field-level parity lives in
    // `bridgeFieldParity.test.ts`.
    const wire = extract(
      source("server/mcp/src/bridge/generated/bridgeRequests.ts"),
      /type: '(vmark\.[^']+)'/g
    );
