 ...
 *   - Converting to a task list PRESERVES an existing checkbox state; only a
 *     line that was not already a task starts unchecked.
 *
 * @coordinates-with listDetection.ts — getListBlockBounds and the line pattern
...
import { getListBlockBounds, isHorizontalRuleLine, LIST_LINE_PATTERN } from "./listDetection";
 ...
 *
 * The three single-line converters above rewrite only the cursor's own marker,
 * which turns one list into three: `- one` / `1. two` / `- three`. A list is one
 * block (see `shared/blockSpan`), so changing its type changes all of it —
 * which is what WYSIWYG has always done.
 *
...
export function convertListBlock(view: EditorView, target: ListTypeTarget): void {
  ...
  for (let n = firstLine; n <= lastLine; n += 1) {
    ...
    if (!match || isHorizontalRuleLine(text)) {
      rewritten.push(text);
      continue;
    }
    ...
    rewritten.push(`${indent}${markerFor(target, indent.length, counters, text)}${content}`);
  }
...
function innermostListRange(
  ...
  let base = Infinity;
  for (let n = selFirst; n <= selLast; n += 1) base = Math.min(base, indentOf(n));
...
function markerFor(
  ...
  if (target === "task") {
    // A line that is ALREADY a task keeps whether it was ticked; converting a
    // plain bullet to a task starts it unchecked.
    const checked = /^\s*[-*+]\s*\[([ xX])\]\s/.exec(originalText)?.[1] ?? " ";
