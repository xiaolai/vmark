 ...
 * `agentCore.src.js` + `agentCoreWalk.src.js` + `agentCoreRoles.src.js` — the
 * accessible-name / visibility functions, the composed walk, and the role
 ...
 * @coordinates-with lib/browser/agent/agentCoreWalk.src.js — its composed walk
 ...
 *   (core + walk + roles), pinned by recorderShimRustParity.test.ts
 * @module lib/browser/agent/agentCore
 */

import CORE from "./agentCore.src.js?raw";
import WALK from "./agentCoreWalk.src.js?raw";
import ROLES from "./agentCoreRoles.src.js?raw";

/** The core asset: the core script, its composed walk, then its role vocabulary —
 *  one string, so every builder that prepends it still ships one role/name/walk
 *  implementation. Function declarations only, `__vmark`-prefixed, ES5. */
