 ...
 * lines are replaced; <br> in other contexts is left as-is.
 */
function cleanBrTagsInTables(markdown: string): string {
 ...
 *
 * Applies in order:
 * 1. Convert <br> to newlines in table rows
 * 2. Strip unnecessary mid-line backslash escapes
 */
export function cleanPastedMarkdown(markdown: string): string {
  if (!markdown) return markdown;

  let result = markdown;
  result = cleanBrTagsInTables(result);
