...
`Mod + Shift + F` 會格式化**選取範圍所跨越的區塊** —— 游標或選取範圍觸及的整個段落、清單或表格，而不是選中的那幾個字元。中日韓間距是相鄰兩個字元**之間**邊界的屬性，半個詞的選取範圍不包含這樣的邊界，因此該命令指定的是要修正的區域，而不是要重寫的文字。沒有選取範圍時，它會格式化游標所在的區塊。

兩個命令保護的內容完全相同（見「受保護內容」），因此先全選再按 `Mod + Shift + F` 是安全的。
...
每次中日韓文格式化之後，格式化器會比對文件的**內容骨架**：去掉空白與標點、並統一字元寬度之後的文字。所有格式化規則改動的只有空白、標點或英數字的寬度，因此這個骨架必須原樣返回 —— 而且它是一個序列而非計數，所以內容被重新排序也會被發現。字母、數字、漢字、假名、諺文和表情符號都計入其中。

若檢查失敗，文件**完全不會被修改**，並會跳出提示告知。拒絕執行絕不會默默發生，也絕不會與「本來就沒有需要修改的地方」混為一談。
