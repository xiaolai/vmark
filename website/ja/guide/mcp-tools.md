...
`workflow_run` は、`source` テキストとして渡したワークフローを AI 所有のタブで実行します。引数：`tabId?`、`source`（ワークフローテキスト——小さな行指向の文法；あなたが書くか、AI が書くか、または [`workflow_record`](#workflow-record) があなた自身の操作からキャプチャします）、`inputs?`（`{name}` 参照に代入される `{name: value}` のマップ）、`allowRepeat?`。これは `{runId, steps}` を **即座に** 返します——複数ステップの実行は 1 回のリクエストより長く続くことがあるため、実行は **非同期** に行われます。進捗は [`browser_read`](#browser-read) の `workflow_status` をポーリングしてください。
...
AI 所有のタブでの **あなた自身の操作** を、再生可能なワークフローとして記録します。引数：`tabId?`、`recordOp`（`"start"` または `"stop"`）、`site?`（記録されたワークフローのフロントマター内のサイト ID；デフォルトは `recording`）。

`start` は `record` 権限によって **同意ゲート** されます。この権限は——`execute_js` や `session` と同じく——**恒久的に付与されることは決してありません**：記録のたびに毎回あらためて確認されるため、AI があなたを黙って記録することはできません。あなたが許可するまで、`start` は `needsApproval` を返します。許可すると、VMark は休止中のページワールドのキャプチャシムを起動し、あなたが行う **クリックとフィールドの編集** の記録を開始します。`stop` は `{source, inputs, eventCount}` を返します——`source` は、保存したり、そのまま [`workflow_run`](#workflow-run) に渡したりできるワークフローテキストです。

記録は **設計上、値を含みません**。これはページを信頼するフィルターではありません：あなたが入力した内容が記録されることは一切ありません。すべてのテキストフィールドは名前付きの `{input}` 変数になり（値は再生時に与えられ、記録されることはありません）、**パスワードまたはワンタイムコードのフィールド** は `confirm:` ステップになります——再生時にあなたが手作業で完了する、人間によるゲートです——そのため秘密情報はパラメータ化すらされません。そしてすべての URL はオリジンとパスだけに切り詰められるため、クエリ文字列内のトークンが残ることはありません。記録されるのは、あなたが触れた **ロケーター**（ARIA ロールとアクセシブルネーム）であり、そのデータではありません。記録はページ遷移をまたいであなたに追従し、上限があります（ページごとに 200 イベント、セッションごとに 1,000）。
