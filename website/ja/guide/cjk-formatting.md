...
`Mod + Shift + F` は**選択範囲がまたぐブロック全体**をフォーマットします — カーソルや選択範囲が触れている段落、リスト、表の全体であり、選択した文字だけではありません。CJK の字間は隣り合う 2 文字の**あいだ**の境界に属する性質で、単語の一部を選んでもその境界は含まれません。そのためこのコマンドは、書き換える文字列ではなく修正する領域を指定します。選択範囲がない場合はカーソル位置のブロックをフォーマットします。

両方のコマンドが保護する対象は同一です（「保護されるコンテンツ」を参照）。したがって全選択してから `Mod + Shift + F` を押しても安全です。
...
CJK フォーマットのたびに、フォーマッターは文書の**コンテンツ・スケルトン**を前後で比較します。空白と句読点を取り除き、文字幅を正規化したテキストです。すべての整形ルールが変更するのは空白・句読点・英数字の幅だけなので、このスケルトンは必ず一致して戻ってこなければなりません。しかも個数ではなく並びなので、内容の入れ替わりも検出できます。英字、数字、漢字、かな、ハングル、絵文字がすべて対象です。

チェックに失敗した場合、文書は**まったく変更されず**、その旨が通知されます。拒否が黙って起きることはなく、「変更すべき箇所がなかった」と混同されることもありません。
