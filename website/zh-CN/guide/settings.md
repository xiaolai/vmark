...
| 关闭时清理未使用的图片 | 自动删除资产文件夹中文档不再引用的图片。关闭文档、窗口或应用时均会执行。同一文件夹中其他文档仍在引用的图片会被保留，被移除的图片会进入系统废纸篓 | 关闭 | 开启/关闭 |
