...
`Mod + Shift + F` 格式化**选区所跨越的块** —— 光标或选区触及的整个段落、列表或表格，而不是选中的那几个字符。中日韩间距是相邻两个字符**之间**边界的属性，半个词的选区不包含这样的边界，因此该命令指定的是要修正的区域，而不是要重写的文本。没有选区时，它格式化光标所在的块。

两个命令保护的内容完全相同（见[受保护内容](#受保护内容)），因此先全选再按 `Mod + Shift + F` 是安全的。
...
每次 CJK 格式化之后，格式化器会比对文档的**内容骨架**：去掉空白与标点、并统一字符宽度之后的文本。所有格式化规则改动的只有空白、标点或字母数字的宽度，因此这个骨架必须原样返回 —— 而且它是一个序列而非计数，所以内容被重新排序也会被发现。字母、数字、汉字、假名、谚文和表情符号都计入其中。

如果检查失败，文档**完全不会被修改**，并会弹出提示告知。拒绝执行绝不会静默发生，也绝不会与「本来就没有需要修改的地方」混为一谈。
