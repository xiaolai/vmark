...
`workflow_run`은 AI 소유 탭에서 `source` 텍스트로 제공하는 워크플로우를 실행합니다. 인수: `tabId?`, `source` (워크플로우 텍스트 — 작은 줄 지향 문법; 당신이 작성하거나, AI가 작성하거나, [`workflow_record`](#workflow-record)가 당신 자신의 동작에서 캡처합니다), `inputs?` (`{name}` 참조에 치환되는 `{name: value}` 맵), `allowRepeat?`. 이는 **즉시** `{runId, steps}`를 반환합니다 — 다단계 실행이 단일 요청보다 오래 지속될 수 있으므로 실행은 **비동기적으로** 이루어집니다. 진행 상황은 [`browser_read`](#browser-read)의 `workflow_status`를 폴링하세요.
...
AI 소유 탭에서 **당신 자신의 동작**을 재생 가능한 워크플로우로 기록합니다. 인수: `tabId?`, `recordOp` (`"start"` 또는 `"stop"`), 그리고 `site?` (기록된 워크플로우의 front-matter 사이트 id; 기본값은 `recording`).

`start`는 `record` 권한에 의해 **동의 게이트** 되며, 이 권한은 — `execute_js` 및 `session`과 마찬가지로 — **결코 상시 부여가 아닙니다**: 모든 기록은 당신에게 매번 새로 요청하므로 AI는 결코 당신을 몰래 기록할 수 없습니다. 당신이 허용하기 전까지 `start`는 `needsApproval`을 반환하며, 허용하고 나면 VMark는 휴면 상태의 페이지 월드 캡처 shim을 준비하고 당신이 수행하는 **클릭과 필드 편집**을 기록하기 시작합니다. `stop`은 `{source, inputs, eventCount}`을 반환합니다 — 이 `source`는 저장하거나 [`workflow_run`](#workflow-run)에 곧바로 넘길 수 있는 워크플로우 텍스트입니다.

기록은 **구성상 값을 담지 않으며**, 이는 페이지를 신뢰하는 필터가 아닙니다: 당신이 입력하는 것은 결코 캡처되지 않습니다. 모든 텍스트 필드는 이름이 지정된 `{input}` 변수가 되고 (그 값은 재생 시 공급되며, 결코 기록되지 않습니다); **비밀번호 또는 일회용 코드 필드**는 `confirm:` 단계 — 재생 시 당신이 직접 손으로 완료하는 인간 게이트 — 가 되므로 비밀은 매개변수화조차 되지 않습니다; 그리고 모든 URL은 origin + path로 축약되므로 쿼리 문자열의 토큰은 살아남을 수 없습니다. 기록되는 것은 당신이 만진 **로케이터** (ARIA 역할 + 접근 가능한 이름)이며, 그 데이터는 결코 기록되지 않습니다. 기록은 페이지 탐색을 넘어 당신을 따라가며 제한됩니다 (페이지당 200개 이벤트, 세션당 1,000개).
