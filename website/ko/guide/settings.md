...
| 연속 줄 바꿈 보존 | 여러 빈 줄을 축소하지 않고 그대로 유지 | 켜기 | 켜기 / 끄기 |
...
| 닫을 때 사용하지 않는 이미지 정리 | 문서가 더 이상 참조하지 않는 에셋 폴더의 이미지를 자동 삭제. 문서, 창, 앱 중 무엇을 닫아도 실행됩니다. 같은 폴더의 다른 문서가 아직 참조하는 이미지는 유지되며, 제거된 이미지는 시스템 휴지통으로 이동합니다 | 끄기 | 켜기 / 끄기 |
...
| 줄 바꿈 축소 | 3개 이상의 연속 줄 바꿈을 2개로 줄이기 | 끄기 | 켜기 / 끄기 |
