...
마지막 세션을 닫으면 패널이 숨겨지지만 세션은 살아있습니다 — `` Ctrl + ` ``로 다시 열면 이전 상태로 돌아갑니다. 셸이 정상 종료(`exit` 또는 `Ctrl + D`)되면 해당 탭이 자동으로 닫히고, 마지막 탭이었다면 패널도 숨겨집니다. 셸이 오류로 종료되면 탭은 종료 코드를 표시한 채 열려 있으며, 아무 키나 눌러 재시작할 수 있습니다.
