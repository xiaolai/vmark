...
- For Phase 2a, the preferred waiver baseline is narrow — per origin edge and
  per upstream revision, with a required reason. When the upstream advances
  further, the edge reopens for a new resolution prompt rather than inheriting
  the waiver.
...
- **O1 — Context composition algebra.** Single-inheritance overlay is the
  ...
  **Resolved (Phase 2a, 2026-07-18):** overlay frozen as designed —
  deterministic child→parent selection, additive-only claim inheritance
  (no child-side hiding), enforcement never inherited
  (`dev-docs/grills/coherence/design-2a.md` D1).
- **O2 — Human-edit input inference quality.** False edges are noise; missed
  ...
  **Resolved (Phase 3 design, 2026-07-19):** query-only proposals from
  the prior-input-set heuristic (context-relative, head-safe, roles
  preserved); confirmation is the normative `provenance-confirmation`
  re-emission; LLM inference deferred until dogfood acceptance data
  exists (`dev-docs/grills/coherence/design-3.md` D1).
...
- **O5 — Canon curation UX.** How claims get extracted from prose, promoted,
  ...
  claims too).
  **Resolved (Phase 2a, 2026-07-18):** four explicit human acts on a
  stable claim id (extract / promote / scope / retire); scoping is
  reversible visibility, retirement is supersession only; nothing
  persists without an explicit human act (design-2a.md D2).
...
- **O8 — Waiver scope semantics.** Per-edge vs. per-object vs. per-claim
  ...
  **Resolved (Phase 2a, 2026-07-18):** per-edge, per-upstream-revision
  with required reason — held; the §9.2 fail-closed projection *is* the
  re-prompt policy (linear advance reopens, divergence stays unwaivable);
  expiry ships as UI in Phase 2b (design-2a.md D3).
...
- **Phase 2a — Semantic-model design phase (no UI). COMPLETE
  (2026-07-18):** O1, O5, O8, and the R33 relation designed as one model
  and owner-approved — normative record in
  `dev-docs/grills/coherence/design-2a.md`; one additive wire change
  (check-result `context` + `claims_fingerprint`, format stays 0).
...
- **Phase 3 — Human-edit inference + git-mapped contexts. DESIGNED
  (2026-07-19):** lazy-confirmation inference, required-expiry
  delegations bound to the authenticated bridge principal (R29), opt-in
  branch contexts with no auto-selection, completed-merge diagnostics —
  normative record in `dev-docs/grills/coherence/design-3.md`; spec
  advances to revision 2 (additive).
...
2. **O1 design session** (context composition) — before Phase 2 is planned in
   detail; Phase 0–1 can be planned immediately. **Done (Phase 2a,
   2026-07-18, design-2a.md).**
