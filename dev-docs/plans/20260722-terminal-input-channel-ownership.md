...
- **R2 — Single PTY writer.** All VMark-generated bytes go through the public
  `term.input(text, true)` → `onData` funnel, never a direct `pty.write`. `term.onData` in
  `terminalSessionInputWiring.ts` becomes the one and only choke point.
...
### WI-3.1: Single PTY write path via `term.input()` (gate path)
- **Goal:** In gate mode, all VMark bytes go through public `term.input(text, true)` → `onData`;
  `onCompositionCommit` stops being a PTY writer.
- **Acceptance:** Byte-identical fixture replay; in gate mode `terminalSessionInputWiring.ts`
  contains **zero dedup conditionals**; the three documented `triggerDataEvent` side effects are
  preserved (verify scroll-to-bottom + cursor-show).
- **Tests (first):** `terminalSessionInputWiring.test.ts` — rewritten around a real instance
  (see WI-5.1); `traceReplay.test.ts` — full byte-identity sweep.
- **Touched areas:** `terminalSessionInputWiring.ts:79-108` (drop the commit write path),
  `setupImeComposition.ts` (commit → `term.input`).
