...
| **SP0** integration gate | ✅ **functional/fault/PERF PASS** | `spike-sp0-operator-slice.md`; the absolute 500k benchmark PASSES — preview p95 0.027 ms (≤ 20 ms), bounded 2-revision sub-dag (`preview.test.rs::perf_500k…`) |
| WI-3.4 perf (accept O(1) + bounded preview) | ✅ **done** | Fix B O(1) accept idem lookup + `load_sub_dag` bounds the preview to the affected objects (was a whole-corpus `load_dag`+clone); 500k benchmark PASS. **Phase-3 gate 8/8 green.** |
...
| Phase 3 surface (UI component) | ⬜ **remaining (UI/live only)** | the preview **React panel** + i18n×10 (visual QA); the WI-3.3 live provider call. The machine-checkable Phase-3 gate is GREEN. |
...
| Multi-object group-commit | 🔴 **NOT ship-ready — G-B re-review 03 (`019f8214`, 10th review): 2 Critical + 3 High** | `accept_group.rs`. Against ratified Option A. Blocking: (C1) `ledger.rs` append never fsyncs the parent dir on new-segment creation → a `group-prepare` manifest in a fresh segment can be lost after power-loss (#5 not durable); (C2) abort of a partial with committed members leaves them live and a retry rejects the committed member as stale — all-or-nothing violated (#6/#7 not closed); (H1) reviewed preview not bound to `group_id`/candidate set → edgeless groups can authorize a mismatched commit; (H2) preflight allows a multi-parent candidate recovery can never reconstruct; (H3) `with_write_lock` flock is Unix-only, no Windows cross-process exclusion. Record: `dev-docs/grills/coherence/durable-store/group-commit-rereview-03.md`. |
| WI-4.1/4.2/4.3 Extract-Canon | 🟠 **redesigned, still NOT ship-ready** | `extract_canon.rs`, 2 tests. Inherits the group-commit's DO-NOT-SHIP status (commits through `accept_group`). #5 preview now surfaces conformance edges but the re-review found the new-edge state must also gate the accept precondition (#8). |
