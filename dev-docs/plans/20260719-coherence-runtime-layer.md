...
| Multi-object group-commit | 🟠 **redesigned, still NOT ship-ready** | `accept_group.rs` + `design-accept-consistency.md`. **G-B re-review (`019f7c7e…`) returned DO-NOT-SHIP (8 MAJOR).** Fix-now set ✅ FIXED + tested: #1/#2 winner-map heal-on-open, #3 append-error poison, #4 group_id full identity, #8 new-edge precondition. Deferred (own design pass + a 3rd review): #5 durable manifest, #6 recovery revalidation, #7 cross-process lock. |
