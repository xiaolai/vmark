...
| Multi-object group-commit | 🟠 **redesigned, still NOT ship-ready** | `accept_group.rs` + `design-accept-consistency.md`. Redesign closed the FIRST review's gaps (group identity folded in idem, preflight-all, defined recovery, #5 preview edges); **the G-B RE-review (`019f7c7e…`) returned DO-NOT-SHIP with 8 MAJOR.** Fix-now (#1/#2/#3 heal+append, #4 group_id full identity, #8 preview precondition): in progress. Deferred (own design pass + 3rd review): #5 durable manifest, #6 recovery revalidation, #7 cross-process lock. |
