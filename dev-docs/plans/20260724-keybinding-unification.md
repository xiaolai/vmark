...
|---|---|---|---|
| 1 | **Mechanic gate does not EXTRACT keymap ids** — `editorMechanics.ts` validated a hand-maintained allowlist only; a new unclassified chord (e.g. `multiCursor/keymap.ts` hardcodes) was invisible. | CONFIRMED → **RESOLVED** | **DONE:** `mechanicExtractionGate.test.ts` source-scans the whole `src/plugins/**` + `src/services/assembly/**` subtree (auto-covers new producers), extracts 68 `getShortcut` ids (all ∈ DEFAULT_SHORTCUTS), 77 command ids (all registered), and 28 mechanic chords, and FAILS on any unclassified id/chord. 8 real structural mechanics added to `APPROVED_MECHANICS`. (Found: `markInputRules` `Mod-b/i` duplicate the rebindable bold/italic — consolidation follow-up; and `source.wordWrapToggle` `Mod-Alt-w` shadows the rebindable `wordWrap` — consolidation follow-up.) |
| 2 | **Phase 8 does not GENERATE** — `check-keybinding-manifest.mjs` was drift-CHECK only; the website table uncovered; Rust checked against `localized.test.rs` (mirror), not the real `localized.rs`. | CONFIRMED → **RESOLVED (gate, not codegen — by design)** | **DONE:** the drift gate now covers **all four real surfaces** fail-closed — frontend defaults + Rust mirror + the REAL `localized/*.rs` `accel(...)` sites (incl. the platform-conditional form) + the website `shortcuts.md` table (order-insensitive). Full manifest→codegen of the hand-written, structured Rust menu builder + prose docs is impractical/fragile; a comprehensive gate achieves WI-8.1's GOAL (no three-file drift) better. Allowed exceptions documented (undo/redo/quit menu-only; heading range). |
...
| 4 | **No `consumer`/`captureOwner` on `shortcutDefinitions` entries** — WI-1.3 built a forward integrity gate only; entries carry no self-classification. | CONFIRMED → **intent met by gates (deliberate deviation)** | The INTENT (classification + integrity) is delivered by three gates — forward integrity (`keybindingReferentialIntegrity`), reverse closure (`shortcutConsumerClosure`, `KNOWN_UNBOUND=[]`), and keymap extraction (`mechanicExtractionGate`). Adding literal per-entry fields to 123 defs would be redundant ceremony that duplicates the gates; `captureOwner` is genuinely context-dependent and lives on the `Binding`. A documented deviation from the literal WI-1.3 text (same call as WI-4.2). |
| 5 | **Dead rebind rows (real UX bug)** — `addCursorAbove/Below`, `skipOccurrence`, `softUndoCursor` appeared rebindable in Settings but `multiCursor/keymap.ts` hardcoded their chords. | CONFIRMED → **RESOLVED (bug killed)** | **DONE:** the multi-cursor keymap now rebuilds live from the shortcuts store (mirrors `editorKeymapExtension`); the four chords resolve via `getShortcut` and are genuinely rebindable without restart. `shortcutConsumerClosure` `KNOWN_UNBOUND` is now `[]` — zero dead definitions. (`selectNextOccurrence`/`selectAllOccurrences` `Mod-d`/`Mod-Shift-l` have no `DEFAULT_SHORTCUTS` rows, so no phantom Settings row — classified as mechanics; making THEM rebindable is a separate feature.) |
...
`shortcutConsumerClosure.test.ts`, now complemented by the keymap-side extraction gate
(`mechanicExtractionGate.test.ts`).

**All 7 findings closed:** #1/#2/#5/#6/#7 fully resolved; #3 (docs amended) + #4 (intent met by
gates) are documented deviations. Net from the "fix everything" pass: 1 real UX bug killed
(dead rebind rows), 1 correctness bug killed (AltGraph), the mechanic gate made real, the
