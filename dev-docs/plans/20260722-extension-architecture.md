...
| WI-4.4 | ✅ **DONE** — `FormatConfig.outline` + `toPlainText`. `OutlineView` and `StatusBarCounts` both ask the format. The metrics segment cache bypasses itself for a non-markdown projection, because its segment splitting and `LIST_START_RE` mirror `stripMarkdown`'s own regexes — markdown's shape, not a general one |
| WI-4.5 | ✅ **DONE, with a distinction the plan collapsed.** Not every markdown default is a bug: `dispatchEditor(null) → markdown` for a NEW UNTITLED file is intended — VMark is a markdown editor, and "New File" must stay markdown. The stale-tab path already fails closed to `WelcomeScreen`. The real failure-open was `Editor.tsx`'s `?? MarkdownEditorSurface`: a format declaring `kind: "wysiwyg"` without a component rendered **as markdown**. `registerFormat` now rejects that at registration, and the fallback is gone |
...
| WI-4.7 | 🟡 **svg dedup done** — `parseSvg` is now the single implementation; the adapter's own `parseXml` plus a second root-element `DOMParser` are gone (3 parses → 1). **Mermaid's three hosts are NOT deduped and arguably should not be**: they share one engine (`renderMermaid` + `sanitizeSvg`) and differ only in mount paradigm — React state, ProseMirror decoration, imperative DOM. Those are genuinely different lifecycles, not copy-paste; collapsing them needs a host abstraction that does not exist and would be a bigger risk than the duplication |
