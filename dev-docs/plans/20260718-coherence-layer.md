---
vmark:
  id: 019f758b-af03-7e53-a6f9-bf1e9468d93e
---
...
- **Status:** **Phases 0 and 1 complete** (2026-07-18).
  ...
  confidence in the synthetic dogfood session. Dogfood session 2
  (self-hosted on this repo's coherence corpus, codex-as-human): M1
  10/10 exact, M2 5/5, M4 5 of ≤5, M5 12.4 s — **PASS WITH
  RESERVATIONS** (fix F2 scan latency, then re-run through MCP
  funnels; see the dogfood log). Codex cross-model review:
  READY TO BUILD (rule 60 §6). Phase 2a design session complete
  (2026-07-18, `design-2a.md` APPROVED). **Phase 2b complete
  (2026-07-19)**: WI-2b.0–2b.10 all landed (`check-coherence-phase.sh 2`
  green); dogfood session 3 PASS — M3 2/2 via real claude-CLI checks,
  both Session-2 reservations closed (ninth edge through the live MCP
  funnel at inferred confidence; F2 re-verified at 234 ms). Phase 3
  decomposed 2026-07-19 (design-3.md APPROVED) and **COMPLETE
  (2026-07-19)**: WI-3.0–3.9 all landed
  (`check-coherence-phase.sh 3` green); dogfood session 4 PASS WITH
  RESERVATIONS — O2 precision 8/8 (1 known-limitation missed edge, LLM
  inference stays deferred), delegated MCP resolve verified live with
  fail-closed refusal + audit chain, branch-context mapping live; F7
  (resolve missing from the live dispatch arm) caught by the dogfood and
  fixed with a dispatch-path regression test. **Next: Phase 4 design
  session** (first non-writing vertical; owner: a larger longitudinal O2
  measurement on newly-introduced dependencies precedes reducing human
  review).
...
| WI-1.10 | Read-only MCP tools on the existing server, Rust-terminal path: sidecar `tools/coherence.ts` (`coherence_status`, `coherence_edges`) + `EXPECTED_TOOL_COUNT` bump + Rust routing in `mcp_bridge/routing.rs::handle_rust_side` reading kernel state (no webview hop); tests at sidecar and Rust layers; errors for invalid/unknown workspace roots (a stale index auto-rebuilds per WI-1.5, never errors) | R23 |
...
## Phase 2a — semantic-model design session — COMPLETE (2026-07-18)

Delivered as `dev-docs/grills/coherence/design-2a.md` (Status: APPROVED —
codex-as-human owner review, APPROVED WITH CHANGES, all 11 required
changes applied; decision record quoted verbatim in the doc). Decisions
D1–D5 resolve O1 (single-inheritance overlay frozen; additive-only claim
inheritance), O5 (four explicit lifecycle acts on a stable claim id;
scoping is reversible visibility, retirement is supersession only), O8
(narrow per-edge/per-revision waivers; §9.2 fail-closed divergence is the
re-prompt policy), and R33 (feed matrix + enforcement decides labeling,
not the feed; explicit confirmation to enforce). One wire change, additive:
`check-result` gains `context` + `claims_fingerprint` (format stays 0).

## Phase 2b — semantic layer (decomposed 2026-07-18 per the 2a amendment)

WIs in dependency order — full table with tests-first anchors in
`design-2a.md` §"Phase 2b decomposition"; the gate
(`scripts/check-coherence-phase.sh 2`) grows per-WI assertions as each WI
lands and runs the suites it names (rule 60 §3).

- **WI-2b.0 — entry gate (R21, contract first):** spec revision 1 of
  format 0 (D1–D5 semantics + additive check-result fields) + paper §O
  amendments + phase-2 gate skeleton. No implementation WI starts before
  this lands.
- **WI-2b.1 — contexts:** manifest load/validate as a delta over Phase 1
  projection (`effective_selection` extends `dag::resolve`/`project_edge`).
- **WI-2b.2 — claims:** lifecycle commands (human actor, in-app only),
  claim-id index, `effective_claims`, deterministic current-entry
  resolution incl. concurrent supersession.
- **WI-2b.3 — projection:** check-result indexing, axis-2 states,
  endpoint-advance expiry, context/fingerprint liveness (pre-v1 results
  historical only).
- **WI-2b.4 — checker:** provider dispatch, tunable τ gate,
  timeout→unknown, evidence extraction, `coherence_check`.
- **WI-2b.5 — breakdown UI:** axis-2 badges, Check action, waiver expiry,
  "previously waived ×N", enforcement-aware labeling.
- **WI-2b.6 — claim UI (minimal)** and **WI-2b.7 — context UI (minimal)**
  with explicit confirmations (enforcement toggle dialog).
- **WI-2b.8 — MCP:** read-only `claims`/`contexts` actions (R23 intact).
- **WI-2b.9 — i18n ×10 + website guides.**
- **WI-2b.10 — dogfood session 3:** M3 ≥ 70% AND Session 2 reservation
  closure (ninth edge, live MCP-funnel run, F2 at scale) gated on
  M1/M2/M4/M5 re-measurement + the qualitative gate.

**Phase 2b DoD:** `bash scripts/check-coherence-phase.sh 2` exits 0 —
fail-closed, runs the suites; WI linkage per rule 60 §2.
## Phase 3 — decomposed 2026-07-19 (design-3.md APPROVED)

Design record: `dev-docs/grills/coherence/design-3.md` (owner review
APPROVED WITH CHANGES; all 12 required changes applied; decision record
quoted verbatim). D1 human-edit inference: context-relative head-safe
proposals (prior-input-set heuristic only; LLM deferred pending O2
data) with `provenance-confirmation` as the sole re-emitting
transformation (fresh id/txf/idem, exact head match). D2 delegation:
required-expiry grants bound to the authenticated bridge principal —
never caller-supplied identity — gating the one mutating MCP action
(`resolve`). D3 branch contexts: opt-in `git_branch`, pull-only
candidate chip, NO auto-selection; completed-merge classifier with
per-merge-hash deduped diagnostics. D4: F5 workspace-precedence
routing (fail-loud ambiguity), F6 ownership-scoped disconnect cleanup.
Spec advances to revision 2 (four additive changes incl. the manifest
unknown-field round-trip guarantee).

WIs in dependency order — full table with tests-first anchors in
`design-3.md` §"Phase 3 decomposition": WI-3.0 entry gate (contract
first, R21) → WI-3.1 proposal/confirmation kernel → WI-3.2 breakdown
confirm flow → WI-3.3 delegation kernel → WI-3.4 grant UI → WI-3.5
mutating MCP + F5/F6 → WI-3.6 branch contexts → WI-3.7 merge surface →
WI-3.8 guides ×10 → WI-3.9 dogfood session 4 (O2 full record,
owner-judged, fail-closed).

**Phase 3 DoD:** `bash scripts/check-coherence-phase.sh 3` exits 0 —
fail-closed, runs the suites; WI linkage per rule 60 §2.

- **Phase 4 (outlined, not decomposed):** first non-writing vertical as
  schema pack + format adapter; MCP hardening. Decomposed by a plan
  amendment after its own design pass.
...
1. ~~Which real creative project is the dogfood workspace?~~ — resolved for
   Phase A: self-host on the VMark repository's coherence corpus; the
   synthetic-workspace fallback remains available for later sessions.
