...
//! (`AsNativeFailure`). Anything unclassified stays `internal`, so an
//! unrecognised failure degrades to today's behavior rather than being guessed
//! at. Every arm's code, kind, i18n key and `mcpCode` is pinned by
//! `ai_guards.test.rs` and `surface_failure.test.rs`.

use crate::browser::ai_guards::with_mcp_code;
use crate::browser::native_failure::{AsNativeFailure, NativeSurfaceError};
...
/// prose, and `detail.detail` keeps the failure's full text reachable. Three
/// classes also carry the `detail.mcpCode` token the MCP client is promised:
/// `UNSUPPORTED_PLATFORM` (the `browser` tool description names it verbatim —
/// audit 20260903 X-04), `DIALOG_NOT_OWNED`, and `STALE_COMMAND` — a gate refusal
/// that once had to cross the string boundary; `eval` now carries the typed
/// refusal intact (`eval_outcome::EvalError::Refused`), so that arm is the
/// classifier honouring a class in the vocabulary, not the path a refusal takes.
pub(in crate::browser) fn surface_failure(error: &(impl AsNativeFailure + ?Sized)) -> CommandError {
    let error = error.as_native_failure();
    let (base, kind, mcp) = classify(&error);
    let classified = base.with_detail(json!({ "kind": kind, "detail": error.to_string() }));
    match mcp {
...
fn classify(error: &NativeSurfaceError) -> (CommandError, &'static str, Option<&'static str>) {
