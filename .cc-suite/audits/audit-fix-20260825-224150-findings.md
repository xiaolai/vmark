...
| 25 | src/services/windowClose/fsChangeHandlers.ts | 198 | High | Refactoring Debt | `handleSemanticBatch` is 66 lines combining partitioning, pair reconstruction, media policy and dispatch | Split into focused helpers | fixed (dispatchRenames + handleMediaChangeEvent extracted; 71 -> 24 lines) | - |
| 26 | src/hooks/useExternalFileChanges.ts | 255 | High | Logic & Correctness | Batches run concurrently; a slower earlier read can overwrite a newer modification with stale content | Serialize batches or use generation tokens | fixed (serialized batch chain + regression test) | - |
| 27 | src/hooks/useExternalFileChanges.ts | 118 | High | Logic & Correctness | Queued conflicts are never revalidated, so a tab closed or renamed during the debounce is resolved on a stale path | Re-check before prompting and before applying | fixed (isQueuedConflictStillLive revalidation) | - |
| 28 | src/hooks/useExternalFileChanges.ts | 264 | High | Logic & Correctness | Cleanup cancels the timer but neither drains the queue nor stops an in-flight batch rearming it after unmount | Give the queue a dispose operation | fixed (BatchQueue.dispose latch) | - |
| 29 | src/hooks/useExternalFileChanges.ts | 171 | High | Refactoring Debt | `handleModifyEvent` is a 58-line high-branch state machine | Extract restoration/reconciliation/action functions | fixed (applyModifyPolicy service; hook 251 -> 195 lines) | - |
...
| 33 | src/lib/formats/adapters/HtmlPreview.tsx | 67 | Medium | Refactoring Debt | 167-line component mixing transition state, grants, sanitization and two frame renderers | Extract a trusted-preview hook and frame components | fixed (useHtmlTrust hook; component 270 -> 161 lines, 1709 tests unchanged) | - |
...
| 38 | src/stores/documentStore/document.ts | 128 | High | Logic & Correctness | `fromUserEdit=false` changes content without bumping the revision, so a client holding the old revision can overwrite the new snapshot | Decouple dirty tracking from revision tracking | rejected on evidence (every other content path bumps; the one exception is a lossless re-render — pinned by revisionCoverage.test.ts) | - |
...
| 42 | src/stores/documentStore/storeContract.ts | 38 | Medium | Logic & Correctness | `documents`/`getDocument` expose mutable `DocumentState`, letting callers bypass actions and revision invalidation | Expose readonly views | fixed (Readonly<DocumentState> on documents + getDocument; 0 call-site changes) | - |
| 43 | src/stores/documentStore/storeContract.ts | 74 | Medium | Dead Code | `setContent` is production-dead, retained only as a legacy test alias | Remove and migrate tests | fixed (97 test call sites migrated, action + contract entry deleted, gate now asserts non-existence) | - |
...
| 46 | vitest.gates.config.ts | 79 | High | Logic & Correctness | The gate tier inherits an app-tier 1.6× oversubscription while each worker spawns subprocesses, multiplying concurrency under check:predelta | Measure a gate-tier worker limit separately | measured, no change needed (8-24 workers all inside run-to-run variance; wall set by a few subprocess-bound files — numbers in vitest.shared.ts) | - |
| 47 | vitest.gates.config.ts | 70 | Medium | Shortcuts & Patches | A suite-wide 60s timeout delays hang detection across all gate tests | Per-test timeouts for the genuinely slow cases | measured, rejected (slowest test 16.5s unloaded; a 25s default would go flaky under predelta contention — reasoning recorded in vitest.gates.config.ts) | - |
| 48 | e2e/lib/vmarkMcp.mjs | 78 | High | Duplication | `portFilePath()` duplicates the sidecar's platform app-data resolution | Share one resolver | resolved by assertion (portFileAgreement.test.mjs pins the two implementations) | - |
| 49 | e2e/lib/vmarkMcp.mjs | 100/132/161/172/206/226 | Medium–High | Logic / Refactoring | Lax port parsing, 133-line function, silent frame discard, post-exit sends, handshake process leak, ignored `structuredContent` | See per-item suggestions in the thread | fixed (strict port parse, noise capture, dead-pipe refusal, handshake reap, structuredContent; jsonRpcStdio extracted + 31 tests) | - |
| 50 | e2e/journeys/01-boot-editor-ready.mjs | 22 | High | Duplication | `list_windows` normalization duplicates other harness code | Share a normalizer from bridge.mjs | fixed (listWindows in bridge.mjs; three copies collapsed, one had drifted) | - |
| 51 | e2e/journeys/01-boot-editor-ready.mjs | 41 | High | Logic & Correctness | `getTabs` excludes the synthetic browser workspace tab, so a browser-active session reads as an empty welcome screen | Include the workspace tab when judging the active surface | fixed (getWorkspaceTab + tabStripHarnessContract.test.tsx) | - |
| 52 | e2e/journeys/01-boot-editor-ready.mjs | 17 | Medium | Refactoring Debt | 49-line `run` mixing bridge parsing, readiness, tab invariants and logging | Extract focused helpers | fixed (activeTab + contentSurface extracted; run() 87 -> 34 lines) | - |
...
| 55 | e2e/wait-ready.mjs | 51 | High | Logic & Correctness | The readiness orchestration itself has no direct tests | Add mock-bridge tests for attempt/poll/cleanup | fixed (mockBridge.mjs + waitReady.test.mjs, 8 orchestration cases) | - |
...
| 57 | src/components/Editor/MediaView/MediaView.tsx | 61 | Medium | Refactoring Debt | 121-line component mixing grant lifecycle, error tracking and three render modes | Extract a hook and a fallback component | fixed (useMediaAsset hook; component 192 -> 146 lines, 20 tests unchanged) | - |
| 58 | src/contexts/useWindowReady.ts | 28 | High | Shortcuts & Patches | A fixed 100ms delay cannot guarantee listener registration under load | Replace with an explicit readiness barrier | fixed (menuCommandsReady barrier replaces the 100ms guess) | - |
| 59 | e2e/journeys/01-boot-editor-ready.mjs | 41 | High | Logic & Correctness | No deterministic tests cover empty/browser/media/split/lazy session shapes | Extract snapshot evaluation and add Vitest cases | fixed (tabStripHarnessContract.test.tsx covers empty/browser/document snapshot shapes deterministically) | - |
